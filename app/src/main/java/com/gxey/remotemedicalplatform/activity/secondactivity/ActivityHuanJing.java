...
import com.gxey.remotemedicalplatform.adapter.HuanJingAdapter;
...
public class ActivityHuanJing extends BaseActivity implements View.OnClickListener {
    ...
    private HuanJingAdapter adapter;
    private List<HuanJingBean> huanJingBeen;
    ...
    protected void initView() {
        ...
        toolbarMid.setText("生活环境");
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayShowTitleEnabled(false);
        ScreenUtils.setStatusBarLightMode(ActivityHuanJing.this, R.color.black);
        huanJingBeen = new ArrayList<>();
        initLoad();
        recyclerViewYichuan.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));
        recyclerViewYichuan.setAdapter(adapter = new HuanJingAdapter(this, huanJingBeen));
...
//        initLoadMoreListener();
        ...
        adapter.setOnItemClickListener(new HuanJingAdapter.OnRecyclerViewItemClickListener() {
            @Override
            public void onClick(View view, HuanJingAdapter.ViewName viewName, int position) {
    ...
    private void getData() {
        ...
        Observable.create(new Observable.OnSubscribe<Integer>() {
            ...
            public void call(Subscriber<? super Integer> subscriber) {
                if (MyHttpHelper.isConllection(ActivityHuanJing.this)) {
                    String[] key = new String[]{};
                    Map<String, String> map = new HashMap<String, String>();
                    String result = MyHttpHelper.GetMessage(ActivityHuanJing.this, UrlConfig.SelEnvironment, key, map);
...
//                                成功
                                ...
                                if (jsonArray.length()>0) {
                                    for (int i = 0; i < jsonArray.length(); i++) {
                                        HuanJingBean huanJingBean =new HuanJingBean();
                                        JSONObject temp = (JSONObject) jsonArray.get(i);
                                        huanJingBean.setId(temp.getString("Id"));
                                        huanJingBean.setVentilationFacilities(temp.getString("VentilationFacilities"));
                                        huanJingBean.setFuelType(temp.getString("FuelType"));
                                        huanJingBean.setAddUserId(temp.getString("AddUserId"));
                                        huanJingBean.setUserId(temp.getString("UserId"));
                                        huanJingBean.setDrinkingWater(temp.getString("DrinkingWater"));
                                        huanJingBean.setLavatory(temp.getString("Lavatory"));
                                        huanJingBean.setLivestockBar(temp.getString("LivestockBar"));
                                        huanJingBeen.add(huanJingBean);
            ...
            public void onNext(Integer integer) {
                switch (integer) {
                    ...
                    case 2:
                        ...
                        ToastUtils.s(ActivityHuanJing.this, msg);
                        break;
                    case 3:
                        swipeYichuan.setRefreshing(false);
                        adapter.notifyDataSetChanged();
                    ...
                    case 4:
                        swipeYichuan.setRefreshing(false);
                        ToastUtils.s(ActivityHuanJing.this, msg);
                        Intent intent = new Intent(ActivityHuanJing.this, LoginActivity.class);
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE && lastVisibleItem + 1 == adapter.getItemCount()) {
                    ...
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            List<HuanJingBean> huanJingBeen = new ArrayList<HuanJingBean>();
                            for (int i = 0; i < 10; i++) {
//                                huanJingBeen.add(new HuanJingBean("神经病", "2018-03-02"));
                            }
                            adapter.AddFooterItem(huanJingBeen);
                            //设置回到上拉加载更多
                            adapter.changeMoreStatus(adapter.PULLUP_LOAD_MORE);
                            Toast.makeText(ActivityHuanJing.this, "更新了 " + huanJingBeen.size() + " 条数据", Toast.LENGTH_SHORT).show();
                        }
                    }, 3000);
