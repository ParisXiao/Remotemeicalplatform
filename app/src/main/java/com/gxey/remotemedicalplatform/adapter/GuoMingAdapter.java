...
public class GuoMingAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {


    private LayoutInflater mInflater;
    private Context context;
    private List<GuoMingBean> guoMingBeen = new ArrayList<>();
    ...
    public GuoMingAdapter(Context context, List<GuoMingBean> guoMingBeen) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof MyViewHolder) {
            ((MyViewHolder) holder).itemTitle.setText("过敏史");
            ((MyViewHolder) holder).itemName.setText("过敏名称");
            ((MyViewHolder) holder).itemTime.setText("过敏时间");
            ((MyViewHolder) holder).itemYichuanYcname.setText(guoMingBeen.get(position).getAllergicDrug());
            ((MyViewHolder) holder).itemYichuanTime.setText(guoMingBeen.get(position).getWarehouseEntryTime());
    ...
    public void AddHeaderItem(List<GuoMingBean> items) {
        guoMingBeen.addAll(0, items);
        notifyDataSetChanged();
    }

    public void AddFooterItem(List<GuoMingBean> items) {
